...
class aruco_detector:
    ...
    def detect_marker_positions(self, img):
        ...
        for i in range(len(ids)):
            ...
            # ------------------- ADD: Covariance based on marker rotation -------------------
            # theta_marker: yaw of marker relative to camera (radians)
            # (can also consider distance or detection confidence)
            rvec = rvecs[i,0]
            theta_marker = np.linalg.norm(rvec)  # rough measure of rotation angle

            base_std = 0.02  # 2cm default per axis
            angle_factor = np.clip(1 + abs(theta_marker)/np.pi * 3, 1, 5)  # inflates with rotation

            cov_local = np.diag([base_std**2, (base_std*angle_factor)**2])
            # -------------------------------------------------------------------------------

            lm_measurement = measure.Marker(lm_bff2d, idi, covariance=cov_local)
