...
class EKF:
    ...
    def __init__(self, robot):
        ...
        #State us a vector[x,y,theta]
        self.robot_init_state = np.zeros((3,1))
        self.lm_pics = []
        for i in range(1, 11):
    ...
    def predict(self, raw_drive_meas):
        ...
        #this prediction step has nothing to do with the landmarks but if the state that already has landmarks, the state transition must have land mark.
        state_bar = self.get_state_vector()# get the whole pass state with landmarks
        state_bar[0:3, 0] = self.robot.state[:, 0]  # update robot part bassed on the drive model
    ...
    def update(self, measurements):
        ...
        y = z - z_hat                       # measurement residual
