...
# import SLAM components (your M2)
...
class Operate:
    def __init__(self, args):
        self.args = args  # store once for safe access
        ...
        if self.args.play_data:
            ...
            self.pibot = PenguinPi(self.args.ip, self.args.port)

        # initialise SLAM parameters
        self.ekf = self.init_ekf(self.args.calib_dir, self.args.ip)
        ...
        if self.args.yolo_model == "":
            self.detector = None
            self.yolo_vis = cv2.imread('pics/8bit/detector_splash.png')
        else:
            self.detector = Detector(self.args.yolo_model)
    ...
    def update_slam(self, drive_meas):
        ...
        # 2) Motion status from current wheel speeds (for gated landmark init)
        v, w = self.ekf.robot.convert_wheel_speeds(drive_meas.left_speed, drive_meas.right_speed)
        moving = (abs(v) > self.ekf.STATIONARY_V_THRESH) or (abs(w) > self.ekf.STATIONARY_W_THRESH)

        # 3) Recovery / toggle logic
        if self.request_recover_robot:
            ...
            # Gate *initialization* of new landmarks by angle & motion (no pause needed)
            self.ekf.add_landmarks(lms, moving=moving)
            # EKF update (for any already-known tags)
            self.ekf.update(lms)

        # 4) HUD: live status
        st = self.ekf.status
        self.notification = f"SLAM {'ON' if self.ekf_on else 'PAUSED'} | moving={st['moving']}  angle_good={st['angle_good']}  can_add={st['can_add']}"
        ...
        if self.command['save_inference']:
            if self.file_output is not None:
                self.pred_fname = self.output.write_image(self.file_output[0],
                                                          self.file_output[1])
                # fixed: refer to self.pred_fname
